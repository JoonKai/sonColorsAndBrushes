...
namespace sonColorsAndBrushes
{
    public class BilinearInterpolation
    {
        ...
        private double BilinearCoeff(double x, double y, double X0, double Y0,
        double X1, double Y1, double C00, double C10,
        double C01, double C11)
        {
            return (Y1 - y) * ((X1 - x) * C00 +
            (x - X0) * C10) / (X1 - X0) / (Y1 - Y0) +
            (y - Y0) * ((X1 - x) * C01 +
            (x - X0) * C11) / (X1 - X0) / (Y1 - Y0);
        }
        public void SetInterpShading(double X0, double Y0, double X1, double Y1,
